...
final class IllusionCloneController {
    private final IllusionSummonerTower owner;
    private final List<CloneInstance> clones = new ArrayList<>();

    IllusionCloneController(IllusionSummonerTower owner) {
        this.owner = owner;
    }

    void enqueue(PlayerLane lane, Tower sourceTower, IllusionProfile profile, List<Vec3> offsets) {
        if (sourceTower == null || profile.cloneCount() <= 0 || sourceTower.health() <= 0.0) {
            return;
        }
        IllusionCloneSpawnQueue.enqueue(owner, lane, sourceTower, profile, offsets);
    }

    void spawnQueued(PlayerLane lane, Tower sourceTower, IllusionProfile profile, Vec3 offset) {
    ...
    void cleanup(PlayerLane lane) {
        IllusionCloneSpawnQueue.cancel(owner);
        for (CloneInstance clone : clones) {
            var entity = lane.arenaWorld().getEntity(clone.entityId());
            if (entity != null) {
                entity.discard();
