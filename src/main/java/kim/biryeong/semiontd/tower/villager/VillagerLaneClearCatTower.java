...
public class VillagerLaneClearCatTower extends EntityBackedTower {
    private double killStackDamage;

    public VillagerLaneClearCatTower(TowerType type, UUID ownerPlayer, TeamId teamId, int laneId, GridPosition position) {
        super(type, ownerPlayer, teamId, laneId, position);
    }

    public VillagerLaneClearCatTower(TowerType type, UUID ownerPlayer, TeamId teamId, int laneId, GridPosition originalPosition, GridPosition currentPosition) {
    ...
    public double modifyAttackDamage(SemionTowerEntity towerEntity, SemionMonsterEntity target, double damageAmount) {
        ...
        if (runtimeMonster != null && runtimeMonster.senderTeam().isEmpty()) {
            return adjustedDamage * (1.0 + value(VillagerAbilityKey.WAVE_BONUS));
        }
    ...
    protected void copyRuntimeStateFrom(Tower previousTower) {
        if (previousTower instanceof VillagerLaneClearCatTower catTower) {
    ...
    private void explode(SemionTowerEntity towerEntity, SemionMonsterEntity target, double damageAmount) {
        ...
        double radius = value(VillagerAbilityKey.EXPLOSION_RADIUS);
