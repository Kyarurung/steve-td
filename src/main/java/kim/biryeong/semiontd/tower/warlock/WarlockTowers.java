...
public final class WarlockTowers {
    ...
    static {
        ...
        TowerDescriptionRegistry.registerTemplate(T2_SLAVE, List.of("<gray><dark_purple>흑마법사</dark_purple>가 데려온 희귀한 양입니다.</gray>", "<gray>사망 시 주위 {ability.deathEffectRadius:number}블록 내 적이 받는 <#ec8d34>피해</#ec8d34>를 해당 라운드 동안 <#ec8d34>{ability.towerDamageTakenBonus:percent}</#ec8d34> 증가시킵니다.</gray>"));
        TowerDescriptionRegistry.registerTemplate(T3_SLAVE, List.of("<gray><dark_purple>흑마법사</dark_purple>가 데려온 양입니다. 희귀했던 색을 잃어 화가 났습니다.</gray>", "<gray>사망 시 주위 {ability.deathEffectRadius:number}블록 내 적이 받는 <#ec8d34>피해</#ec8d34>를 해당 라운드 동안 <#ec8d34>{ability.towerDamageTakenBonus:percent}</#ec8d34> 증가시킵니다.</gray>"));
        TowerDescriptionRegistry.registerTemplate(T2_RANGED_SLAVE, List.of("<gray><dark_purple>흑마법사</dark_purple>가 키우는 개구리입니다.</gray>", "<gray>사망 시 주위 {ability.deathEffectRadius:number}블록 내 적의 <#ffe78d>공격 속도</#ffe78d>를 해당 라운드 동안 <#ffe78d>{ability.attackSpeedReduction:percent}</#ffe78d> 감소시킵니다.</gray>"));
        TowerDescriptionRegistry.registerTemplate(T3_RANGED_SLAVE, List.of("<gray><dark_purple>흑마법사</dark_purple>가 키우는 개구리입니다.</gray>", "<gray>사망 시 주위 {ability.deathEffectRadius:number}블록 내 적의 <#ffe78d>공격 속도</#ffe78d>를 해당 라운드 동안 <#ffe78d>{ability.attackSpeedReduction:percent}</#ffe78d> 감소시킵니다.</gray>"));
    }
    ...
    private static List<String> rangedWarlockDescription() {
        ...
        return WarlockConfig.AWAKENING_ENABLED ? lines : lines.subList(0, lines.size() - 2);
    }
    ...
    private static String damageSoftCapDescription() {
        return "<gray>흡수로 얻는 추가 피해는 <#ec8d34>" + globalAbility(DAMAGE_SOFT_CAP, "number") + "</#ec8d34>까지 그대로 적용되며, 이후 완만하게 증가합니다.</gray>";
    }
