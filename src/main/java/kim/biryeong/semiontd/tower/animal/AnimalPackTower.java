...
abstract class AnimalPackTower extends EntityBackedTower {
    private final AnimalPackController pack = new AnimalPackController(this);

    protected AnimalPackTower(TowerType type, UUID ownerPlayer, TeamId teamId, int laneId, GridPosition position) {
        super(type, ownerPlayer, teamId, laneId, position);
    }

    protected AnimalPackTower(
            TowerType type,
            UUID ownerPlayer,
            TeamId teamId,
            int laneId,
    ...
    protected final double leaderValue(AnimalAbilityKey ability) {
        return AnimalConfig.RUNTIME.value(leaderType(), ability);
    }
