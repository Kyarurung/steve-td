...
| 개인 보정 즉시 적용 | 낮음 | 표본과 검증 없이 켜면 메타 왜곡 가능성이 크다. | 최소 200-500경기 로그를 모은 뒤 dry-run과 실제 결과 예측력을 비교한다. |
...
- rating 보정은 바로 켜지 않고 debug/dry-run 리포트로만 검증한다.
...
- rating update dry-run에서 개인 성과 보정이 전체 ELO 변화량의 10-20% 상한을 넘지 않는다.
...
| lane leak 지표가 불공정하게 작동함 | 원시 누수 수 대신 wave/견제 압박으로 보정하고, 충분한 로그 전에는 rating에 적용하지 않는다. |
